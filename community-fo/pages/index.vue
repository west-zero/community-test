<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <h2 class="text-2xl font-bold text-slate-800">최근 게시물</h2>
      <NuxtLink v-if="isLoggedIn" to="/write" class="btn btn-primary flex items-center space-x-2">
    ...
    <div v-else-if="posts.length === 0" class="card p-12 text-center">
      ...
      <p class="text-slate-500">아직 게시물이 없습니다.</p>
    </div>
        ...
        <div class="flex flex-col gap-3">
          <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div class="flex-1 min-w-0">
              <span class="inline-block px-2 py-1 text-xs font-medium bg-primary-100 text-primary-700 rounded-full mb-2">
                {{ post.menuName }}
              </span>
...
const loadPosts = async (page = 0) => {
  loading.value = true
  const result = await api.get<PageResponse>(`/api/posts?page=${page}&size=10`)
